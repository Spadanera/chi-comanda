...
ordersRouter.get("/:eventid/:destinationsids", authorizationMiddleware([Roles.bartender, Roles.checkout, Roles.waiter]), async (req: Request, res: Response) => {
    try {
...
// order items API
ordersRouter.get("/:id/items", authorizationMiddleware([Roles.bartender, Roles.checkout, Roles.waiter]), async (req: Request, res: Response) => {
    try {

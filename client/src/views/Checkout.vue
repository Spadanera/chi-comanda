...
async function getTables() {
  ...
  if (tables.value.length && tables.value[0].status === 'ACTIVE') {
    if (selectedTable.value.length === 0) {
      ...
      selectedTable.value = [foundTable ?? tables.value[0]]
    }
  } else {
    selectedTable.value = []
  }
...
function itemRemovedHandler(data: number) {
  const table = tables.value.find((o: Table) => o.items.find((i: Item) => i.id === data))
  if (table) {
    table.items = copy<Item[]>(table.items.filter((i: Item) => i.id !== data))
  }
}
...
<template>
  <v-navigation-drawer v-if="props.event?.id" v-model="drawer" mobile-breakpoint="sm">
    ...
    <v-list v-model:selected="selectedTable" lines="two">
      <v-list-item :key="table.id" v-for="table in sortedTables" :value="table"
